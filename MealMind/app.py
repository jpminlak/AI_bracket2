...
async def recommend_next(req: RecommendRequest, next_meal: str = Query("lunch"), live: bool = Query(False)):
    """
    next_meal: "breakfast" / "lunch" / "dinner"
    """
    goal = calc_goal_calories(req)
    if live:
        js = await llm_complete_open(build_prompt_open(req, goal))
        if isinstance(js, dict) and {"breakfast", "lunch", "dinner"}.issubset(js.keys()):
            try:
                full = _ensure_shape_and_fit(js, goal_kcal=goal)
            except Exception:
                full = _fallback_menu(goal)
    else:
        full = _fallback_menu(goal)

    # ---- 여기서 다음 한 끼만 추출하고 리스트로 변환 ----
    next_meal_key = next_meal.lower()
    if next_meal_key not in {"breakfast", "lunch", "dinner"}:
        next_meal_key = "lunch"
    next_meal_data = full[next_meal_key]

    menu_list = [x.strip() for x in next_meal_data["menu"].split(",")]
    single = {
        next_meal_key: {
            "menu_list": menu_list,
            "kcal": next_meal_data["kcal"],
            "nutrients": next_meal_data["nutrients"]
        },
        "total_kcal": next_meal_data["kcal"],
        "goal_kcal": next_meal_data["kcal"],
        "reason": full["reason"]
    }
